...
import (
	...
	cloud "github.com/xuyaming0800/terraform-provider-teststack/teststack"
)
...
const (
	cloudMark    = "teststack"
	cloudTitle   = "Teststack"
	cloudPrefix  = cloudMark + "_"
	docRoot      = "/website"
	providerRoot = "/teststack"
	exampleRoot  = "/example"
)
...
func main() {
	...
	for _, product := range products {
		if resource != "" && product.Resource != cloud.HumpToDownLine(resource) {
			continue
		}
					...
					if f1.IsDir() {
						...
						for _, f2 := range fs2 {
							if strings.HasPrefix(f2.Name(), "data_source_") {
								...
								product.DataSourceExample = pwd + exampleRoot + "/" + "data" + cloud.DownLineToHump(product.DataSource)
								...
								product.ResourceExample = pwd + exampleRoot + "/" + cloud.DownLineToHumpAndFirstLower(product.Resource)
							}
						}
...
func genDoc(pwd string, product, docType, filename, name string, resource *schema.Resource, example string) {
	...
	if docType == "resource" {
		data["description"] = "Provides a resource to manage " + cloud.DownLineToSpace(data["resource"])
		data["description_short"] = data["description"]
	} else {
		data["description"] = "Use this data source to query detailed information of " + cloud.DownLineToSpace(data["resource"])
		data["description_short"] = data["description"]
	}
